    ...
    if (value.length) {
      ...
      if (closestMatch) {
        ...
        $('#search-category').append(/*html*/ `
            <div id='search-category-item' class='flex cursor-pointer group px-1 hover:bg-sky-400 hover:text-white-50  items-baseline gap-1' data-value='${closestMatch.canonical_id}'>
            <i
...
async function markAndPanToSearchResult(selectedResult) {
  clearTimeout(locToCategoryTimer);
  ...
  if (feature_type === 'country') {
    await updateChosenCountryState(context.country.country_code);
  } else {
    if (chosenCountryISO) {
      await updateChosenCountryState();
    }
